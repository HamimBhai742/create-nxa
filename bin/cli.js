...
// Helper function to copy template directory
...
async function getProjectName() {
  ...
  return new Promise((resolve) => {
    rl.question(`❓ ${bold(cyan('What is your project name?'))} ${dim('(my-nxa-app)')}: `, (answer) => {
      rl.close();
      const sanitized = answer.trim();
      resolve(sanitized || 'my-nxa-app');
    });
  });
}

async function main() {
  const targetDirName = await getProjectName();
